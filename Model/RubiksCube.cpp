//
// Created by Aditya Parmar on 26-12-2023.
//
...
//
//  Describe the move using the index
//
...
//
//  Perform a move in the Rubik's Cube
//
...
//
//  Invert the move in the Rubik's Cube
//
...
vector<RubiksCube :: MOVE > RubiksCube::randonShuffleCube(unsigned int times) {
    vector<MOVE> moves_performed;
    srand(time(0));
    for(unsigned int i=0;i<times;i++){
        unsigned  int selectMove = (rand()%18); // randomly select one of the 18 moves
        moves_performed.push_back(static_cast<MOVE>(selectMove)); // push back the move to the vector
        this->move(static_cast<MOVE>(selectMove)); // performs the move in the cube
    }
